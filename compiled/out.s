...
/*
********HAM PRIME**********
Compiled with love on Mon Jan 27 2025 21:24:12 GMT-0700 (Mountain Standard Time)
**************************
*/
...
# popping multi-line clobbers
	...
	#//------- line: name : "stu" , age : 8 ------- #
	#//------- line: name : "vwx" , age : 9 ------- #
	#//------- line: { ; Person < name,:,__STRING3__,,,age,:,1 > , ; Person < name,:,__STRING4__,,,age,:,3 > , ; Person < name,:,__STRING5__,,,age,:,4 > , ; Person < name,:,__STRING6__,,,age,:,5 > , ; Person < name,:,__STRING7__,,,age,:,6 > , ; Person < name,:,__STRING8__,,,age,:,7 > , ; Person < name,:,__STRING9__,,,age,:,8 > , ; Person < name,:,__STRING10__,,,age,:,9 > , ; } ------- #
	# {
		# pushing clobbers
		# 
		pushl $0
		...
		call __rc_requestOwnership__
		add $8, %esp
		# optimized move from 3 to 4(%esi)
		movl $3, 4(%esi)
		# pushing clobbers
		push %esi
		push %ecx
		...
		call __rc_requestOwnership__
		add $8, %esp
		# optimized move from 4 to 4(%edi)
		movl $4, 4(%edi)
		# pushing clobbers
		push %esi
		push %ecx
		...
		# requesting ownership (setting sub property)
		# POOOP
		mov __TEMP32_0__, %edx; lea 0(%edx), %edx # awful fix. Need to find some way to declob regs
		push %edx
		pushl %eax
		call __rc_requestOwnership__
		add $8, %esp
		mov __TEMP32_0__, %edx # awful fix. Need to find some way to declob regs
		# optimized move from 5 to 4(%edx)
		movl $5, 4(%edx)
		# pushing clobbers
		push %esi
		push %ecx
		...
		# requesting ownership (setting sub property)
		# POOOP
		mov __TEMP32_1__, %edx; lea 0(%edx), %edx # awful fix. Need to find some way to declob regs
		push %edx
		pushl %eax
		call __rc_requestOwnership__
		add $8, %esp
		mov __TEMP32_1__, %edx # awful fix. Need to find some way to declob regs
		# optimized move from 6 to 4(%edx)
		movl $6, 4(%edx)
		# pushing clobbers
		push %esi
		push %ecx
		...
		# requesting ownership (setting sub property)
		# POOOP
		mov __TEMP32_2__, %edx; lea 0(%edx), %edx # awful fix. Need to find some way to declob regs
		push %edx
		pushl %eax
		call __rc_requestOwnership__
		add $8, %esp
		mov __TEMP32_2__, %edx # awful fix. Need to find some way to declob regs
		# optimized move from 7 to 4(%edx)
		movl $7, 4(%edx)
		# pushing clobbers
		push %esi
		push %ecx
		...
		# requesting ownership (setting sub property)
		# POOOP
		mov __TEMP32_3__, %edx; lea 0(%edx), %edx # awful fix. Need to find some way to declob regs
		push %edx
		pushl %eax
		call __rc_requestOwnership__
		add $8, %esp
		mov __TEMP32_3__, %edx # awful fix. Need to find some way to declob regs
		# optimized move from 8 to 4(%edx)
		movl $8, 4(%edx)
		# pushing clobbers
		push %esi
		push %ecx
		...
		pop %ecx
		pop %esi
		mov %eax, __TEMP32_4__ # Local allocation address for Person
		# char lit to string (explicit format init)
		pushl $__STRING10__
		call cptos
		add $4, %esp
		# requesting ownership (setting sub property)
		# POOOP
		mov __TEMP32_4__, %edx; lea 0(%edx), %edx # awful fix. Need to find some way to declob regs
		push %edx
		pushl %eax
		call __rc_requestOwnership__
		add $8, %esp
		mov __TEMP32_4__, %edx # awful fix. Need to find some way to declob regs
		# optimized move from 9 to 4(%edx)
		movl $9, 4(%edx)
	# }
	# Allocating array {;%ecx,;%esi,;%edi,;__TEMP32_0__,;__TEMP32_1__,;__TEMP32_2__,;__TEMP32_3__,;__TEMP32_4__,;}
	# pushing clobbers
	push %esi
	push %ecx
	push %edi
	...
	add $8, %esp
	# Moving arr to out
	# optimized move from %eax to __TEMP32_5__
	mov %eax, __TEMP32_5__
	#//------- line: ( __TEMP32_5__ ) ------- #
	#//------- line: create people <- __TEMP32_5__ ------- #
	# creating variable "people" of type "array:dynamic" stack?=true
	# Loading local variable "people" @-4(%ebp) with "__TEMP32_5__"
	# optimized move from __TEMP32_5__ to -4(%ebp)
	# setting register "d" to "__TEMP32_5__"
	movl __TEMP32_5__, %edx
	mov %edx, -4(%ebp)
